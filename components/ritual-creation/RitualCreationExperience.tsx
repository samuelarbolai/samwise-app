...
// Distinct from ritual-call's storage key so the two pages don't fight over
// the same slot. A user who used /ritual-call yesterday and /ritual-creation
// today should see their last-pasted link on each.
const DOC_LINK_STORAGE_KEY = 'ritual-creation:docLink';
const REGISTER_RITUAL_URL =
  'https://registernewritual-b6fhjlgejq-uc.a.run.app';

export function RitualCreationExperience() {
  ...
  const start = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/ritual-creation/init', {
        method: 'POST',
...
function PasteLinkForm({
  ...
  return (
    ...
    >
      <h1 className="text-3xl tracking-tight">Build your ritual</h1>
      <p className="text-muted-foreground">
        Paste the link to your ritual document. The clinician will guide you through shaping your
        mantras, your protection, and the practice that lets you face what you have been avoiding.
      </p>
      <input
        autoFocus
        type="url"
