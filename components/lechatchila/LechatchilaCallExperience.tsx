...
const DOC_LINK_STORAGE_KEY = 'lechatchila:docLink';
const REGISTER_RITUAL_URL =
  'https://registernewritual-b6fhjlgejq-uc.a.run.app';

export function LechatchilaCallExperience() {
  ...
  useEffect(() => {
    try {
      ...
      // localStorage unavailable — fall through to blank form.
    }
  }, []);
  ...
  const start = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/lechatchila/init', {
        method: 'POST',
    ...
    try {
      ...
      try {
        ...
        // Browser blocked autoplay — mic button will re-trigger via gesture.
      }
...
function PasteLinkForm({
  ...
  return (
    ...
    >
      <h1 className="text-3xl tracking-tight">Lechatchila</h1>
